...
@router.delete("/working-sets/{working_set_id}")
def delete_working_set_endpoint(working_set_id: int, settings: SettingsDep) -> dict[str, bool]:
    """Delete one durable working set."""
    with db.core_connection(settings) as conn:
        try:
            working_sets.delete_working_set(working_set_id=working_set_id, conn=conn)
        except ValidationError as exc:
            _raise_working_set_http_exception(exc)
    return {"deleted": True}
...
def remove_working_set_item_endpoint(
    ...
    with db.core_connection(settings) as conn:
        try:
            working_sets.remove_working_set_item(
                working_set_id=working_set_id,
                item_id=item_id,
                conn=conn,
            )
