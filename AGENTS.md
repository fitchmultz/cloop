...
- Work in Python 3.14 (match `pyproject.toml`). Always use `uv` commands: `uv sync --dev`, `uv run <tool>`.
...
- Replace string flags with `Literal`/`Enum`; brand IDs with `NewType`.
- Use `Protocol` for pluggable components; mark overrides with `@override` (or equivalent comment for pyright).
- Return immutable data (`tuple`, frozen DTOs) unless mutation is required.
- Use `match` + `Never` to make exhaustiveness explicit.
