...
- **Continuity delivery diagnostics contract**: `/loops/continuity/debug/delivery-decisions` and push-notification selection must stay on the shared bounded delivery-read contract in `src/cloop/storage/continuity_store.py`; pagination is an opaque `cursor`, not `after_outcome_id`.
