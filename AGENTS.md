...
| Continuity diagnostics CLI + MCP | `src/cloop/cli_package/parsers/continuity.py`, `src/cloop/cli_package/continuity_commands.py`, `src/cloop/mcp_tools/continuity_tools.py` |
...
- **Process cleanup hygiene**: if an agent starts `uvicorn`, `vite`, `cloop-scheduler`, the pi bridge, browser automation (the `agents-browser` skill, `peekaboo`, or Playwright only when used through the programmatic test framework), XCTest, or similar long-lived tooling for this repo, it must stop those processes and remove any repo-owned temp profiles, temp workspaces, simulators, or other ephemeral runtime artifacts before ending the session; failed tests or interrupted runs do not excuse leaving them behind. Use `make cleanup-runtime` / `make verify-runtime-clean` when practical, and prefer fixture/finalizer, `finally`, or trap-based teardown over happy-path-only cleanup.
...
- **Durable continuity source of truth**: backend-owned landed outcomes, grouped workflow threads, durable anchors, and durable last-seen markers now live in `src/cloop/storage/continuity_store.py` behind `/loops/continuity*`, while browser-local cache + pending sync behavior live in `frontend/src/continuity-intelligence.ts`; do not revert to localStorage-only continuity truth.
- **Continuity delivery diagnostics contract**: HTTP `/loops/continuity/debug/delivery-decisions`, CLI `cloop continuity delivery-decisions`, MCP `continuity.delivery_decisions`, and push-notification selection must stay on the shared bounded delivery-read contract in `src/cloop/storage/continuity_store.py`; pagination is an opaque `cursor`, push scans walk later pages until they find enough sendable records or hit the bounded budget, and the diagnostics payload should stay lean (`limit`, `truncated`, `continuation`, `decisions`) instead of reviving redundant count metadata.
- **Primary next-move source of truth**: deterministic operator recommendations should be synthesized from the canonical ranked follow-through feed in `frontend/src/continuity-follow-through.ts` via `frontend/src/continuity-recommendations.ts`, then rendered consistently in `frontend/src/shell-operator-cards.ts` and `frontend/src/command-palette.ts` instead of per-surface ad-hoc ranking rules.
- **Continuity recovery source of truth**: explicit replacement, launch-fallback, home-fallback, and working-set-scope recovery plans should be synthesized in `frontend/src/continuity-recovery.ts` and attached through `frontend/src/continuity-follow-through.ts`; operator cards, the receipt rail, and command-palette continuity commands should render that shared recovery contract instead of inventing per-surface stale-target copy.
