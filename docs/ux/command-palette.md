...
- Quick actions include loop capture, direct memory creation, planning-session creation, working-set pinning, working-set activation/focus, and selected-loop mutations (complete, drop, status transitions, snooze, enrich).
- Search covers local loop/object matches, saved views, saved sessions, working sets, memory search, and recall-launch queries for grounded chat/documents.
- Recent command usage remains browser-local and still influences ranking, but it complements — rather than defines — the canonical landed-outcome continuity feed.
