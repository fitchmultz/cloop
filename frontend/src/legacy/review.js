...
export async function launchSavedSession({ reviewKind, sessionId }) {
  if (reviewKind === 'relationship') {
    const snapshot = await selectRelationshipSession(sessionId);
    if (!snapshot) {
      throw new Error(`Failed to load relationship review session ${sessionId}.`);
    }
    return snapshot;
  }
  if (reviewKind === 'enrichment') {
    const snapshot = await selectEnrichmentSession(sessionId);
    if (!snapshot) {
      throw new Error(`Failed to load enrichment review session ${sessionId}.`);
    }
    return snapshot;
  }
  throw new Error(`Unsupported review session kind: ${reviewKind}`);
}
