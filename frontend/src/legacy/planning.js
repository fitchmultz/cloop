...
function renderLaunchSurfaceCard(surface) {
  const web = surface?.web && typeof surface.web === 'object' ? surface.web : {};
  const http = surface?.http && typeof surface.http === 'object' ? surface.http : {};
  const mcp = surface?.mcp && typeof surface.mcp === 'object' ? surface.mcp : {};
  const canOpenReviewSession =
    web.surface === 'review_session'
    && typeof web.review_kind === 'string'
    && parseInteger(web.session_id) != null;

  return `
    <article class="planning-follow-up-card planning-launch-surface-card">
      <div class="planning-follow-up-card-header">
        <div>
          <p class="support-eyebrow">Next operator surface</p>
          <h5>${escapeHtml(surface.label || `Surface #${surface.resource_id ?? '—'}`)}</h5>
        </div>
        <span class="review-session-chip">${escapeHtml(surface.surface || 'surface')}</span>
      </div>
      <p>${escapeHtml(surface.reason || 'Open the next deterministic surface created by this checkpoint.')}</p>
      <div class="review-session-chip-group">
        ${http.path ? `<span class="review-session-chip">HTTP: ${escapeHtml(http.path)}</span>` : ''}
        ${mcp.tool ? `<span class="review-session-chip">MCP: ${escapeHtml(mcp.tool)}</span>` : ''}
      </div>
      ${canOpenReviewSession ? `
        <button
          type="button"
          class="secondary"
          data-action="planning-launch-surface"
          data-surface="${escapeHtml(surface.surface || '')}"
          data-review-kind="${escapeHtml(String(web.review_kind))}"
          data-session-id="${escapeHtml(String(web.session_id))}"
        >
...
function renderFollowUpResourceCard(resource) {
  const details = resource?.details && typeof resource.details === 'object' ? resource.details : {};
  const detailBits = [];

  if (details.review_kind) {
    detailBits.push(`Kind: ${details.review_kind}`);
  }
  if (details.query) {
    detailBits.push(`Query: ${details.query}`);
  }
  if (typeof details.loop_count === 'number') {
    detailBits.push(`Queued loops: ${details.loop_count}`);
  }
  if (details.description) {
    detailBits.push(`Description: ${details.description}`);
  }

  return `
    <article class="planning-follow-up-card">
      <div class="planning-follow-up-card-header">
        <div>
          <p class="support-eyebrow">${escapeHtml(resource.role || 'resource')}</p>
          <h5>${escapeHtml(resource.label || `${resource.resource_type} #${resource.resource_id}`)}</h5>
        </div>
        <span class="review-session-chip">${escapeHtml(resource.resource_type || 'resource')}</span>
      </div>
      <p>${escapeHtml(resource.operation_summary || 'Created during checkpoint execution.')}</p>
      <div class="review-session-chip-group">
        <span class="review-session-chip">Operation ${escapeHtml(String((resource.operation_index ?? 0) + 1))}</span>
        <span class="review-session-chip">${escapeHtml(resource.operation_kind || 'operation')}</span>
        ${detailBits.map((bit) => `<span class="review-session-chip">${escapeHtml(bit)}</span>`).join('')}
      </div>
    </article>
  `;
}

function renderRollbackCues(item) {
  const rollbackCues = item?.rollback_cues && typeof item.rollback_cues === 'object'
    ? item.rollback_cues
    : null;
  const operations = Array.isArray(rollbackCues?.operations) ? rollbackCues.operations : [];
  if (!rollbackCues || !operations.length) {
    return '';
  }
  ...
  return `
    <section class="planning-section-block">
      <div class="planning-section-heading">
        <h5>Rollback cues</h5>
        <span class="cohort-count">${Number(rollbackCues.rollback_action_count || 0)} actions</span>
      </div>
      <div class="review-session-chip-group">
        <span class="review-session-chip">Rollback-supported operations: ${Number(rollbackCues.rollback_supported_operation_count || 0)}</span>
        <span class="review-session-chip">Undoable operations: ${Number(rollbackCues.undoable_operation_count || 0)}</span>
      </div>
      <div class="planning-rollback-list">
        ${operations.map((operation) => `
          <article class="planning-rollback-item">
            <strong>${escapeHtml(operation.summary || operation.kind || 'Operation')}</strong>
            <div class="review-session-chip-group">
              <span class="review-session-chip">${escapeHtml(operation.kind || 'operation')}</span>
              <span class="review-session-chip">${operation.undoable ? 'Undo snapshot kept' : 'No undo snapshot'}</span>
              <span class="review-session-chip">${operation.rollback_supported ? 'Rollback supported' : 'No rollback actions'}</span>
              <span class="review-session-chip">Actions: ${Number(operation.rollback_action_count || 0)}</span>
            </div>
          </article>
        `).join('')}
      </div>
...
function renderExecutionHandoff(item) {
  const followUpResources = Array.isArray(item?.follow_up_resources) ? item.follow_up_resources : [];
  const launchSurfaces = Array.isArray(item?.launch_surfaces) ? item.launch_surfaces : [];

  return `
    ${launchSurfaces.length ? `
      <section class="planning-section-block">
        <div class="planning-section-heading">
          <h5>Next operator surfaces</h5>
          <span class="cohort-count alert">${launchSurfaces.length}</span>
        </div>
        <div class="planning-follow-up-grid">
          ${launchSurfaces.map((surface) => renderLaunchSurfaceCard(surface)).join('')}
        </div>
      </section>
    ` : ''}
    ${followUpResources.length ? `
      <section class="planning-section-block">
        <div class="planning-section-heading">
          <h5>Created follow-up resources</h5>
          <span class="cohort-count">${followUpResources.length}</span>
        </div>
        <div class="planning-follow-up-grid">
          ${followUpResources.map((resource) => renderFollowUpResourceCard(resource)).join('')}
        </div>
      </section>
    ` : ''}
    ${renderRollbackCues(item)}
  `;
}
  ...
  return `
    <div class="planning-execution-history-list">
      ${history.map((item) => {
        ...
        const followUpCount = Number(summary.follow_up_resource_count || 0);
        const launchCount = Array.isArray(item?.launch_surfaces) ? item.launch_surfaces.length : 0;
        const rollbackCount = Number(item?.rollback_cues?.rollback_action_count || 0);
          ...
          <section class="planning-execution-history-item">
            <div class="planning-execution-history-header">
              ...
              <div class="review-session-chip-group">
                ...
                <span class="review-session-chip">Follow-up resources: ${followUpCount}</span>
                <span class="review-session-chip">Launch surfaces: ${launchCount}</span>
                <span class="review-session-chip">Rollback actions: ${rollbackCount}</span>
                <span class="review-session-chip">Executed ${formatRelativeTimestamp(item.executed_at_utc)}</span>
              </div>
            </div>
            ${renderExecutionHandoff(item)}
...
async function executePlanningSession() {
  ...
  try {
    ...
    const launchSurface = Array.isArray(payload?.execution?.launch_surfaces)
      ? payload.execution.launch_surfaces[0]
      : null;
    setPlanningStatus(
      launchSurface
...
async function launchPlanningSurface(button) {
  const reviewKind = button?.dataset?.reviewKind;
  const sessionId = parseInteger(button?.dataset?.sessionId);
  if (!reviewKind || sessionId == null) {
    setPlanningStatus('Launch surface is missing session details.', { isError: true });
    return;
  }
  if (typeof review.launchSavedSession !== 'function') {
    setPlanningStatus(
      'Review-session launcher is unavailable. Open the saved session from the Review selectors.',
      { isError: true },
    );
    return;
  }
  ...
  try {
    await review.launchSavedSession({ reviewKind, sessionId });
    setPlanningStatus(`Opened ${reviewKind} review session #${sessionId}.`);
  } catch (err) {
    console.error('launchPlanningSurface error:', err);
    setPlanningStatus(err.message || 'Failed to open saved review session.', { isError: true });
  }
}
...
function handlePlanningDetailClick(event) {
  ...
  if (action === 'planning-launch-surface') {
    void launchPlanningSurface(button);
  }
}
