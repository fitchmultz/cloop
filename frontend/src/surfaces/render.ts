 ...
 *
 * Scope:
 *   - DOM rendering helpers only.
 *
 * Usage:
 *   - Imported by surface loop/next/timer modules.
 *
 * Invariants/Assumptions:
 *   - API calls stay in api.ts.
 *   - State coordination stays in state.ts and feature modules.
 */

import type { CommentNode, SurfaceLoop } from "./contracts";
import {
 ...
 */
export function statusSelectOptions(current: SurfaceLoop["status"]): string {
 ...
 */
export function queueNextActionResize(root: ParentNode & { querySelectorAll?: typeof document.querySelectorAll }): void {
  const targets = root.querySelectorAll
    ? root.querySelectorAll<HTMLTextAreaElement>('[data-field="next_action"]')
    : [];
...
function isCompactLoop(loop: SurfaceLoop): boolean {
  return new Set(["completed", "dropped"]).has(loop.status);
}

function hasLongMobileCardText(compactCard: boolean, capturedText: string, summary: string): boolean {
  if (compactCard) {
    return false;
  }
...
function buildDueEditor(loop: SurfaceLoop): string {
...
export function setCompactCardExpanded(card: HTMLElement | null | undefined, expanded: boolean): void {
  ...
  if (toggle) {
    card.querySelectorAll<HTMLElement>('[data-action="toggle-compact"]').forEach((button) => {
  ...
  card.querySelectorAll<HTMLElement>('[data-action="remove-tag"], [data-action="edit-tags"]').forEach((button) => {
...
export function setMobileCardTextExpanded(card: HTMLElement | null | undefined, expanded: boolean): void {
...
export function setDueEditorExpanded(card: HTMLElement | null | undefined, expanded: boolean): void {
...
function formatLoopStateLabel(value: unknown): string {
  return String(value ?? "")
    .replace(/_/g, " ")
    .replace(/\b\w/g, (char) => char.toUpperCase());
}

function buildLoopContextBadges(loop: SurfaceLoop): string {
  const badges = [];
...
function renderNextLoop(loop: SurfaceLoop): HTMLElement {
...
export function renderInboxEmptyState(
  { query = "", status = "open", tag = "" }: { query?: string; status?: string; tag?: string } = {},
): HTMLElement {
 ...
 */
export function renderLoop(loop: SurfaceLoop, options: { surface?: "next" } = {}): HTMLElement {
  ...
  if (dueTimeInput instanceof HTMLInputElement) {
    const initialTime = loop.due_date ? "" : localTimeInputValueFromIso(loop.due_at_utc);
    dueTimeInput.value = initialTime;
    dueTimeInput.dataset["initialTime"] = initialTime;
  }
  ...
  if (completionInput instanceof HTMLInputElement) {
 ...
 */
export function renderComment(comment: CommentNode, loopId: number | string, isReply = false): string {
