...
def test_chat_completion_retries_same_selector_when_no_fallback_available(
    ...
    settings = _configure_env(monkeypatch, tmp_path, CLOOP_PI_MODEL="zai/glm-5.2")
    runtime = _QueuedRuntime(
        sessions=[
            _ImmediateFailingSession(
                BridgeUpstreamError("provider_timeout", "timeout", retryable=True)
        ...
        resolutions=[
            _resolution(
                requested_selector="zai/glm-5.2",
                requested_selectors=("zai/glm-5.2",),
                resolved_selector="zai/glm-5.2",
            ),
            _resolution(
                requested_selector="zai/glm-5.2",
    ...
    assert runtime.resolve_requests[1]["selectors"] == ("zai/glm-5.2",)
