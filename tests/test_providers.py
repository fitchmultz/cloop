"""Unit tests for embeddings-only provider kwargs resolution.

Purpose:
    Test API key and base URL resolution for embedding providers.

Responsibilities:
    - Verify correct kwargs returned for supported embedding providers
    - Verify ValueError raised for missing required embedding config
...
Non-scope:
    ...
    - Generative pi model resolution
"""
...
from cloop.embedding_providers import resolve_embedding_provider_kwargs
...
class TestOllamaProvider:
    ...
    def test_raises_when_api_base_missing(self, test_settings: Callable[..., Settings]) -> None:
        ...
        with pytest.raises(ValueError, match="ollama/.*requires.*CLOOP_OLLAMA_API_BASE"):
            resolve_embedding_provider_kwargs("ollama/llama3", settings)
    ...
    def test_returns_api_base_when_configured(self, test_settings: Callable[..., Settings]) -> None:
        ...
        result = resolve_embedding_provider_kwargs("ollama/llama3", settings)
...
class TestGeminiProvider:
    ...
    def test_raises_when_api_key_missing(self, test_settings: Callable[..., Settings]) -> None:
        """Gemini embeddings require CLOOP_GOOGLE_API_KEY."""
        settings = test_settings(google_api_key=None)
        with pytest.raises(
            ValueError, match="Gemini embedding model requires.*CLOOP_GOOGLE_API_KEY"
        ):
            resolve_embedding_provider_kwargs("gemini/gemini-pro", settings)
    ...
    def test_returns_api_key_with_gemini_prefix(
        ...
        result = resolve_embedding_provider_kwargs("gemini/gemini-pro", settings)
    ...
    def test_returns_api_key_with_google_prefix(
        ...
        result = resolve_embedding_provider_kwargs("google/gemini-pro", settings)
    ...
    def test_no_api_base_in_kwargs(self, test_settings: Callable[..., Settings]) -> None:
        """Gemini embeddings never include api_base in kwargs."""
        settings = test_settings(google_api_key="test-key")
        result = resolve_embedding_provider_kwargs("gemini/gemini-pro", settings)
...
class TestOpenAIProvider:
    ...
    def test_raises_when_api_key_missing(self, test_settings: Callable[..., Settings]) -> None:
        ...
        with pytest.raises(
            ValueError, match="OpenAI embedding model requires.*CLOOP_OPENAI_API_KEY"
        ):
            resolve_embedding_provider_kwargs("openai/gpt-4", settings)
    ...
    def test_returns_api_key_with_openai_prefix(
        ...
        result = resolve_embedding_provider_kwargs("openai/gpt-4", settings)
    ...
    def test_returns_api_key_with_gpt_prefix(self, test_settings: Callable[..., Settings]) -> None:
        ...
        settings = test_settings(openai_api_key="sk-test-key")
        result = resolve_embedding_provider_kwargs("gpt-4o-mini", settings)
        assert result == {"api_key": "sk-test-key"}
    ...
    def test_returns_api_key_with_o1_prefix(self, test_settings: Callable[..., Settings]) -> None:
        ...
        settings = test_settings(openai_api_key="sk-test-key")
        result = resolve_embedding_provider_kwargs("o1-preview", settings)
        assert result == {"api_key": "sk-test-key"}
...
class TestLMStudioProvider:
    ...
    def test_returns_empty_kwargs_when_no_api_base(
        ...
        result = resolve_embedding_provider_kwargs("lmstudio/llama3", settings)
        assert result == {}
...
class TestOpenRouterProvider:
    ...
    def test_returns_empty_kwargs_when_no_api_base(
        ...
        result = resolve_embedding_provider_kwargs("openrouter/anthropic/claude-3", settings)
        assert result == {}
...
class TestUnknownModels:
    ...
    def test_returns_empty_kwargs_for_unknown_prefix(
        ...
        """Unknown prefixes return empty kwargs for the embeddings path."""
        settings = test_settings()
        result = resolve_embedding_provider_kwargs("unknown/model", settings)
        assert result == {}
    ...
    def test_returns_empty_kwargs_for_bare_model(
        ...
        settings = test_settings()
        result = resolve_embedding_provider_kwargs("llama3", settings)
        assert result == {}
